...
package mx.collections
{
		...
		implements ICollectionView, IList, IMXMLObject
	{
		...
		public function getItemIndex(item:Object):int
		{
			...
			if (localIndex && sort)
			{
				var startIndex:int = findItem(item, spark.collections.Sort.FIRST_INDEX_MODE);
				if (startIndex == -1)
					return -1;
				
				var endIndex:int = findItem(item, spark.collections.Sort.LAST_INDEX_MODE);
			...
			if (localIndex)
			{
				...
				for (var i:int = 0; i < items.length; i++)
				{
					...
					if (filterFunction == null || filterFunction(item))
					{
						if (sort)
						{
							loc = findItem(item, spark.collections.Sort.ANY_INDEX_MODE, true);
							if (firstOne)
							{
								addLocation = loc;
								firstOne = false;
...
class ListCollectionViewCursor extends EventDispatcher implements IViewCursor
{
	...
	public function findAny(values:Object):Boolean
	{
		...
		try
		{
			index = lcView.findItem(values, spark.collections.Sort.ANY_INDEX_MODE);
		}
		catch(e:SortError)
		{
	...
	public function findFirst(values:Object):Boolean
	{
		...
		try
		{
			index = lcView.findItem(values, spark.collections.Sort.FIRST_INDEX_MODE);
		}
	...
	public function findLast(values:Object):Boolean
	{
		...
		try
		{
			index = lcView.findItem(values, spark.collections.Sort.LAST_INDEX_MODE);
		}
		catch(sortError:SortError)
		{
			//this is because the find critieria is not compatible with the
